...
void vidurkioSkaiciavimas(list <mokinys> &Stud)
{
    int suma;
    ...
    it=Stud.begin();
       while(it!=Stud.end())
            {
              suma=0;
                for(int ii=0; ii<it->C.size(); ii++)
                {
                    suma+=it->C[ii];
                }
                if(suma == 0)
                {
                    it->galutinis = 0.6*(it->egzaminas);
                }else{
                    it->vidurkis = 1.0*suma/it->C.size();
                    it->galutinis = (0.4*(it->vidurkis))+(0.6*(it->egzaminas));
                }
                it++;
          }
}
